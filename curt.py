...
def trace_end():
	...
	print "TODO: wrap up active task statistics..."
	for task in task_info.keys():
		print "%s: %s" % (task, task_state[task]['mode'])
		...
		for task in sorted(tidlist):
			if task_info[task]['pid'] == pid:
				...
				for cpu in sorted(task_state[task]['sys'].keys()):
					print "\t[%6s] %-20s %3u %12.6f %12.6f %12.6f" % (task, comm, cpu, task_state[task]['user'][cpu], task_state[task]['sys'][cpu], task_state[task]['idle'][cpu])
					task_user += task_state[task]['user'][cpu]
					task_sys += task_state[task]['sys'][cpu]
					task_idle += task_state[task]['idle'][cpu]
				print "\t[%6s] %-20s ALL %12.6f %12.6f %12.6f %5.1f%% %6u" % (task, comm, task_user, task_sys, task_idle, (task_user + task_sys) * 100 / (task_user + task_sys + task_idle) if task_user + task_sys > 0 else 0, task_state[task]['migrations'])
				print
				for id in task_state[task]['count'].keys():
					print "\t\t(%3u)%-20s %6u %12.6f+%12.6f %12.6f %12.6f %12.6f" % (id, syscall_name(id), task_state[task]['count'][id], task_state[task]['elapsed'][id], task_state[task]['pending'][id], task_state[task]['elapsed'][id]/task_state[task]['count'][id] if task_state[task]['count'][id] > 0 else 0, task_state[task]['min'][id], task_state[task]['max'][id])
				print
				proc_user += task_user
				proc_sys += task_sys
				proc_idle += task_idle
		print "\t[   ALL] %12.6f %12.6f %12.6f %5.1f%%" % (proc_user, proc_sys, proc_idle, (proc_user + proc_sys) * 100 / (proc_user + proc_sys + proc_idle) if proc_user + proc_sys > 0 else 0)
